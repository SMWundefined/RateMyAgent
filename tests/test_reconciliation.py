...
class TestStrictnessDocIsGenerated:
    ...
    # **No `assets/` gate: this one reads only tracked files.** It opens
    # `tools/schema_strictness.py` and never the asset it used to be gated on,
    # so the gate was costing a CI check for nothing. The other eight in this
    # file do read `assets/` and are left alone; guarding inside the body is a
    # separate change. Same lesson as `test_the_gate_carries_no_skip_marker`
    # below -- a gate that skips in CI is a check that does not run.
