...
def _advertised(tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """What `tools/list` shows this copy's caller. GATE S COPY ONLY.

    **The agent's copy advertises the write tool and nothing else.** The read
    tools exist for the oracle, which reads through its own `--role oracle`
    copy; the agent never needs them. Advertising them anyway made Gate S's
    three arms see different tool sets: the SDK runners filter to `event`, but
    Claude Code's `--tools ""` removes only its built-ins, so its model saw
    `effects`/`effects_array` and was denied them at use time in 4 of 5 phase 2
    chaos runs (GATE-S-PHASE2 section 9.1). Hiding them here makes restriction
    by omission true in all three arms, at the source.

    **Advertising only.** A call to `effects` by name is still served exactly as
    before, and still never advances the generation under `--role agent`
    (`_oracle_read`). This copy diverges from `tests/fixtures/` deliberately;
    the version phase 2 ran is archived as `phase2/event_twin_mcp_server.as-run.py`.
    """
    if ROLE == ROLE_AGENT:
